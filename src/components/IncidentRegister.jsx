      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            Tracked operational incidents, root cause analyses, and resolution registers for <strong className="text-indigo-500">{activeClient}</strong>.
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1100px] border-collapse text-left text-xs sm:text-sm">
            <thead className="border-b border-slate-200 bg-slate-100 text-xs font-bold uppercase tracking-wide text-slate-700 dark:border-slate-800 dark:bg-slate-800/80 dark:text-slate-300">
              <tr>
                {['Incident ID', 'Client', 'Site / Country', 'Incident Date', 'Resolved Date', 'Environment', 'Severity', 'Summary', 'Status', 'Action'].map((label) => <th key={label} className="px-4 py-3.5">{label}</th>)}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {incidents.length === 0 ? (
                ...
                <tr key={incident.id || incident.incidentId} className="align-top transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="whitespace-nowrap px-4 py-3.5 font-mono font-semibold text-indigo-700 dark:text-indigo-400">{valueOf(incident, 'incidentId')}</td>
                  <td className="px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'client')}</td>
                  <td className="px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'siteCountry')}</td>
                  <td className="whitespace-nowrap px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'incidentDate')}</td>
                  <td className="whitespace-nowrap px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'resolvedDate')}</td>
                  <td className="px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'environment')}</td>
                  <td className="px-4 py-3.5"><SeverityBadge severity={incident.severity} /></td>
                  <td className="min-w-64 px-4 py-3.5 text-slate-700 dark:text-slate-300">{valueOf(incident, 'summary')}</td>
                  <td className="px-4 py-3.5"><StatusBadge status={incident.status} /></td>
                  <td className="px-4 py-3.5"><button type="button" onClick={() => setSelectedIncident(incident)} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100"><Eye className="h-3.5 w-3.5" /> View Details</button></td>
                </tr>
              ))}
            </tbody>
          </table>
